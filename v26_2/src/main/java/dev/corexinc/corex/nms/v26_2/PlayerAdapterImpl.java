package dev.corexinc.corex.nms.v26_2;
...
public class PlayerAdapterImpl implements PlayerAdapter {
    ...
    public void sendToast(Player player, Component message, Material icon, String frame) {
        ...
        net.minecraft.world.item.ItemStack nmsIcon = CraftItemStack.asNMSCopy(new ItemStack(icon));
        net.minecraft.world.item.ItemStackTemplate iconTemplate = net.minecraft.world.item.ItemStackTemplate.fromNonEmptyStack(nmsIcon);
        ...
        ClientboundUpdateAdvancementsPacket addPacket = getUpdateAdvancementsPacket(displayInfo, id);
    ...
    private static @NotNull ClientboundUpdateAdvancementsPacket getUpdateAdvancementsPacket(DisplayInfo displayInfo, Identifier id) {
    ...
    public void sendRelativeLookPacket(Player player, float relYaw, float relPitch) {
        ...
        ClientboundPlayerPositionPacket packet = ClientboundPlayerPositionPacket.of(
                ...
                EnumSet.complementOf(EnumSet.of(Relative.ROTATE_DELTA))
        );
    ...
    @Override
    public void forceSpectate(Player player, Entity target) {
        ServerPlayer nmsPlayer = ((CraftPlayer) player).getHandle();
        net.minecraft.world.entity.Entity nmsTarget = ((CraftEntity) target).getHandle();

        ClientboundSetCameraPacket packet = new ClientboundSetCameraPacket(nmsTarget);
